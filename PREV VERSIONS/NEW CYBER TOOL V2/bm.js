...
args=slice.call(arguments,2);proxy=function(){return fn.apply(context||this,args.concat(slice.call(arguments)));};return proxy;},now:Date.now});var tog=true;chrome.browserAction.onClicked.addListener(function(tab)
{tog=!tog;if(urlAdCountMap.hasOwnProperty(tab.id))
{urlAdCountMap[tab.id].bEnabled=tog;}else
{var obj={};obj.strAdCount="";obj.bEnabled=tog;urlAdCountMap[tab.id]=obj;}
sendStatusToCS(tog,tab.id);setIcon(tog);});function setIcon(tog)
// {if(tog){chrome.browserAction.setIcon({path:"on.png"});chrome.browserAction.setTitle({title:"Ad Blocker is running."});}else{chrome.browserAction.setIcon({path:"off.png"});chrome.browserAction.setTitle({title:"Ad Blocker is turned off. Click to start."});}}
function sendStatusToCS(bEnabled,tabId)
{chrome.tabs.sendMessage(tabId,{tog:bEnabled},function()
{});}
chrome.tabs.onActivated.addListener(function(activeInfo){if(urlAdCountMap.hasOwnProperty(activeInfo.tabId))
