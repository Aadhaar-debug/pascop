...
function theBlocking() {
	...
	if(semanticParse("Dark") > 2 || semanticParse(".onion") > 2 || semanticParse("onion") > 2 || semanticParse("Dark Web") > 2 || semanticParse("haystack") > 2 || semanticParse("Whitehousemarket") > 2) {
		// if we detected porn...
		...
		if(isURLWhiteList() == false) 
			blockAccess();
			alert("SITE LEADS TO A DARK WEB PORTAL")
	}
}
...
function semanticParse(keyword) {
	...
	if (typeof jQuery != 'undefined') {  
		...
		if(found.length) {
			var content_count = 0;
			if(keyword == ".onion") content_count = (found.text().match(/.onion/gi) || []).length;
			else if(keyword == "onion") content_count = (found.text().match(/onion/gi) || []).length;
			else if(keyword == "Dark Web") content_count = (found.text().match(/Dark Web/gi) || []).length;
			else if(keyword == "Deep Web") content_count = (found.text().match(/Deep Web/gi) || []).length;
			else if(keyword == "torrent") content_count = (found.text().match(/torrent/gi) || []).length;
			else if(keyword == "Dark") content_count = (found.text().match(/Dark/gi) || []).length;
...
function isURLWhiteList() {
	...
	var whitelist = [ "google.", "facebook.com", "baidu.com", "yahoo.", 
