...
function theBlocking() {
	...
	if(semanticParse("torrent") > 3 || semanticParse("porn") > 3 || semanticParse("jizz") > 3 || semanticParse("fuck") > 10 || semanticParse("sex") > 10) {
		// if we detected porn...
		...
		if(isURLWhiteList() == false) 
			blockAccess();
			alert("SITE BLOCKED BY THE ADMINISTRATOR")
	}
}
...
function semanticParse(keyword) {
	...
	if (typeof jQuery != 'undefined') {  
		...
		if(found.length) {
			...
			else if(keyword == "milf") content_count = (found.text().match(/milf/gi) || []).length;
